...
local font_flags_name = {
	...
	u8"Обводка", --обводка
    u8"Тень", --тень
    u8"Подчеркнутый", --подчеркнутый
    u8"Зачеркнутый" --зачеркнутый
}
...
function EXPORTS.aboutme()
    ...
    sampAddChatMessage(u8'Мой ID: '..id, -1)
    sampAddChatMessage(u8'Мой ник: '..sampGetPlayerNickname(id), -1)
    sampAddChatMessage(u8'Мой лвл: '..sampGetPlayerScore(id), -1)
    sampAddChatMessage(u8'Мой пинг: '..sampGetPlayerPing(id), -1)
    sampAddChatMessage(u8'Мое здоровье: '..sampGetPlayerHealth(id), -1)
    sampAddChatMessage(u8'Моя броня: '..sampGetPlayerArmor(id), -1)
end
